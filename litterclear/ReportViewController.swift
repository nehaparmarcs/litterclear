    ...
    @IBAction func doReport(_ sender: UIButton) {
        if let img = photoImageView.image {
            if let imgData = UIImageJPEGRepresentation(img, 0.2) {
                ...
                //Get report location
                //let reportLoc = CLLocation(latitude: self.latitude, longitude: self.longitude)
                //distanceValidBetweenTwoLocations(source: initialLocation, destination: reportLoc)
                //let validDistance = distanceValidBetweenTwoLocations(source: reportLoc, destination: currentLoc)
                ...
                /*}
                    ...
                    print("Distance from location should not be more than 30 feet")
                    return
                }*/
